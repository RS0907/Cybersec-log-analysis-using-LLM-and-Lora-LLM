...
for idx, chunk in enumerate(read_log_in_chunks(LOGS_PATH, chunk_size=100), start=1):
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            # First attempt uses cache. Retry attempts force a new generation.
            ignore_cache = (attempt > 0)
            response = analyze_logs_with_ollama(SYSTEM_PROMPT, prepared, ignore_cache=ignore_cache)

            json_text = extract_json(response)
            if not json_text:
                if attempt < MAX_RETRIES - 1:
                    print(f"    [Retry] No JSON found in chunk {idx}. Requesting fresh generation...")
                    continue
                else:
            ...
            if not isinstance(parsed, list):
                if attempt < MAX_RETRIES - 1:
                    print(f"    [Retry] Invalid JSON structure in chunk {idx}. Requesting fresh generation...")
                    continue
                else:
                    print(f" Invalid JSON structure in chunk {idx}, skipping.")
                    break
            ...
            if attempt < MAX_RETRIES - 1:
                print(f"    [Retry] Raw JSON decode error in chunk {idx}: {e}. Requesting fresh generation...")
                continue
            else:
                print(f"!.Failed to process chunk {idx} after {MAX_RETRIES} attempts.")
                break
        except Exception as e:
            # For non-JSON errors (like Ollama connection 500 error), just break so we don't spam 3 errors in a row
...
print("LLAMA model used for this analysis.")
